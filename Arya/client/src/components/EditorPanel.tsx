...
interface EditorPanelProps {
  ...
  onUpdate: (updates: Partial<SelectedElementData> & { styles?: Partial<SelectedElementData['styles']> }) => void;
  onClose: () => void;
}
...
const EditorPanel = ({ selectedElement, onUpdate, onClose }: EditorPanelProps) => {
    ...
    return (
    <div className='absolute top-4 right-4 z-50 w-80 rounded-lg border border-gray-200 bg-white p-4 text-gray-900 shadow-xl'>
      ...
      <div className='mb-3'>
        ...
        <textarea value={values.text || ''} onChange={(e) => handleChange('text', e.target.value)} className='w-full rounded-md border border-gray-400 p-2 text-sm outline-none focus:ring-2 focus:ring-indigo-500' rows={3} />
      </div>
